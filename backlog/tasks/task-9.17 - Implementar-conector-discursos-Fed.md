---
id: TASK-9.17
title: Implementar conector discursos Fed
status: To Do
assignee: []
created_date: '2026-06-15 02:49'
labels:
  - international
  - fed
  - speeches
  - connectors
  - model-medium
dependencies: []
references:
  - analysis/source_research_fed.md
parent_task_id: TASK-9
priority: medium
ordinal: 77000
---

## Description

<!-- SECTION:DESCRIPTION:BEGIN -->
Consumir pagina oficial anual de speeches Fed y clasificar discursos por speaker, titulo, fecha y tags editoriales relevantes.
<!-- SECTION:DESCRIPTION:END -->

## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 El conector devuelve speeches normalizados con fecha, speaker, titulo, URL y tags
- [ ] #2 Incluye fixtures/tests offline para listado anual y detalle
- [ ] #3 Permite priorizar discursos por speaker o tema sin resumir extensivamente
<!-- AC:END -->
